...
if(isset($_POST['update_details'])){
    ...
    if($matched_user == "" || $matched_user == $userLoggedIn){
        ...
        $message = "このメールアドレスは既に登録されています。<br><br>";
    }
}else{
    $message="";
}
    ...
    if(md5($old_password) == $db_password){
        if($new_password_1 == $new_password_2){
            if(strlen($new_password_1) <= 4){
                $password_message = "パスワードは5文字以上で入力してください。<br><br>";
            }else{
                $new_password_md5 = md5($new_password_1);
                $password_query   = mysqli_query($con, "UPDATE users SET password='$new_password_md5' WHERE username='$userLoggedIn'");
                $password_message = "パスワードを更新しました。<br><br>";
            }
        }else{
            $password_message = "新しいパスワードが一致していません。<br><br>";
        }
    }else{
        $password_message = "現在のパスワードが間違っています。<br><br>";
    }
}else{
    $password_message = "";
}

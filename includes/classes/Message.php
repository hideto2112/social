...
class Message {
    ...
    public function getMostRecentUsers(){
    ...
    public function sendMessage($user_to, $body, $date){
        if($body != ""){
            $userLoggedIn = $this->user_obj->getUsername();
            $query        = mysqli_query($this->con, "INSERT INTO messages VALUES('', '$user_to', '$userLoggedIn', '$body', 
                '$date', 'no', 'no', 'no')");
        }
    }
    ...
    public function getMessages($otherUser){
        ...
        $get_message_query = mysqli_query($this->con, "SELECT * FROM messages WHERE (user_to='$userLoggedIn' 
        ...
        while($row = mysqli_fetch_array($get_message_query)){
            ...
            $div_top = ($user_to == $userLoggedIn) ? "<div class='message' id='green'>" : "<div class='message' id='blue'>";  // ?->true, :->else
    ...
    public function getLatestMessage($userLoggedIn, $user2){
        ...
        $start_date    = new DateTime($row['date']);       // 送信日時
    ...
    public function getConvosDropdown($data, $limit){
        ...
        $set_viewed_query = mysqli_query($this->con, "UPDATE messages SET viewd='yes' WHERE user_to='$userLoggedIn'");
        ...
        foreach($convos as $username){
            ...
            $is_unreaded_query = mysqli_query($this->con, "SELECT opened FROM messages WHERE user_to='$userLoggedIn' AND user_from='$username' ORDER BY id DESC");
            $row               = mysqli_fetch_array($is_unreaded_query);
            ...
            if($count > $limit){
                $return_string .= "<input type='hidden' class='nextPageDropDownData' value='" . ($page + 1) ."'>
                                   <input type='hidden' class='nextPageDropDownData' value='false'>";
            }else{
                $return_string .= "<input type='hidden' class='nextPageDropDownData' value='true'>
                                   <p style='text-align: center;'>メッセージは以上です。</p>";
            }
